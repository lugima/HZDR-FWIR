...
# Function that writes the mass calibrated data in a .asc file
def write_calibrated(filename, x, y):

    filename = 'mass_intens_data_' + filename

    with open(filename, 'w') as file:
        header = 'Mass (u)' + '\t' + 'I_FC (nA)'
        file.write(header + '\n')
...
data_dir = dir + '\\' + data_fname
masses_dir = dir + '\\' + masses_fname

# Extracts the data from both files

mass_exp, i_fc, mass_tab, name = read_raw_data(data_dir, masses_fname, a, b, c)

# Makes a plot

make_plot(plot_title, mass_exp, i_fc, mass_tab, name)

# Writes calibrated mass data in files
i_fc -= 1e-12 # Remove the correction for logscale

write_calibrated(data_fname, mass_exp, i_fc)